...
<body class="body2">
				...
				<div class="assetbox">
					...
					<div class="deposit_warp4">
						<div class="assettitle">
							<span class="wallet_sub"> USDT <spring:message code="wallet.p2p.sell"/></span>
						</div>
						<div class="buy_list_btnwarp">
							<a href="/global/user/p2pbuy.do" class="buysellbtn w-button"><spring:message code="wallet.p2p.buy"/></a> 
							<a href="#" class="buysellbtn sell w-button"><spring:message code="wallet.p2p.sell"/></a>
						</div>
										...
										<div class="asset_table">
											<div class="asset_tablelist">
												...
												<div class="list_block2">
													<a href="javascript:p2pPop(${item.idx})" class="buysellbtn sell w-button"><spring:message code="wallet.p2p.sell"/></a>
												</div>
											</div>
										</div>
									</c:forEach>
			...
			<div class="buylist_pop">
				<div class="buylist_popbox">
					<div class="buylist_popsection1">
						...
						<div class="pop_warn"><spring:message code="wallet.p2p.sellPop_1"/></div>
					</div>
					...
					<div class="buylist_popsection2">
						<div class="form-block-19 w-form">
							<div><spring:message code="trade.avaWallet"/></div>
							<div class="buylist_popinput">
								<input type="text" class="text-field-26 w-input" id="wdWallet" style="background-color:transparent;" readonly>
								<div class="text-block-43-copy">USDT</div>
							</div>
							<div><spring:message code="wallet.p2p.sellPop_2"/></div>
							<div class="buylist_popinput">
								<input type="text" class="text-field-27 w-input" maxlength="11" id="popInput" onkeyup="setDouble(this,4)"> 
								<a href="javascript:allInput()" class="button-55 w-button"><spring:message code="wallet.p2p.sellAll"/></a>
								<div class="text-block-43">USDT</div>
							</div>
							<div><spring:message code="wallet.p2p.sellPop_3"/></div>
							<div class="buylist_popinput">
								<input type="text" class="text-field-26 w-input" maxlength="256" id="popGet" style="background-color:transparent;" readonly>
								<div class="text-block-43-copy">KRW</div>
							</div>
						</div>
						<a href="javascript:submit()" class="button-56 w-button"><spring:message code="wallet.p2p.sell"/></a>
					</div>
				</div>
			</div>
		</div>
	...
	<script>
		...
		function p2pPop(tidx){
			...
			$("#wdWallet").val(fmtNum(toFixedDown(withdrawWallet,5)));
		}
		function popClose(){
			selectTrader = -1;
			$("#popName").text("");
		...
		function allInput(){
			$("#popInput").val(toFixedDown(Number($("#popQty").text()),4));
			$("#popInput").trigger("input");
		}
		...
		function buyCheck(){
			...
			var getKRW = inputVal * Number(selectPrice);
			if(isNaN(inputVal)) {
				return false;	
			} else if(isNaN(getKRW)){
				return false;
			} 
			else if(getKRW < selectLLimit){
				alert("<spring:message code='wallet.p2p.lLimitErr'/>");
				return false;
			}else if(getKRW > selectMLimit){
				alert("<spring:message code='wallet.p2p.mLimitErr'/>");
				return false;
			}else if(inputVal > Number($("#popQty").text())){
		...
		$(function() {
			$("#popInput").on("input", function() { // 주문 수량 입력 체크
				var input = $(this).val();
				$("#popGet").val(fmtNum(toFixedDown(input * selectPrice,0)));
			});
		});
		...
		function submit(){
			...
			var withdrawMoney = $("#popInput").val();
			...
			var allData = {"withdrawMoney":withdrawMoney, "tidx":selectTrader};
			jQuery.ajax({
				type : "POST",
				data : allData,
				url : "/global/user/withdrawProcessP2P.do",
				dataType : "json",
