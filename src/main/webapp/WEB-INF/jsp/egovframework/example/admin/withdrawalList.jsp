...
<body id="page-top">
					...
					<div class="row">
						<div class="col-lg-12">
							...
							<a href="/global/0nI0lMy6jAzAFRVe0DqLOw/account/realBalance.do?coinname=ETH&type=1">ETH</a> -->
								<div class="card-body">
									<form
										action="/global/0nI0lMy6jAzAFRVe0DqLOw/trade/withdrawalList.do"
											...
											<tbody>
												<c:forEach var="item" items="${list}" varStatus="i">
													<tr style="background-color:${item.color}">
														...
														<td onclick="location.href='/global/0nI0lMy6jAzAFRVe0DqLOw/user/userDetail.do?idx=${item.wuseridx}'" style="cursor: pointer;">${item.name}</td>
														...
														<td>
															...
															<a target="_blank" href="/global/0nI0lMy6jAzAFRVe0DqLOw/trade/withdrawalList.do?pageIndex=1&sdate=&edate=&searchSelect=idx&search=${item.wuseridx }&coin=${item.wcoinname }&wstat=1">
														...
														<td><a target="_blank"
															href="/global/0nI0lMy6jAzAFRVe0DqLOw/trade/emailList.do?idx=${item.wuseridx}">${item.wemail}</a>
														</td>
													</tr>
												</c:forEach>
											</tbody>
	...
	function checkRequest(widx, stat) {
		...
		jQuery.ajax({
			type : "POST",
			url : "/global/0nI0lMy6jAzAFRVe0DqLOw/trade/withdrawalProcess.do?widx=" + widx
					+ "&stat=" + stat + "&tx=" + tx,
			dataType : "json",
			date : {
				widx : widx,
	...
	function changeAlarm(idx, alarm){
		$.ajax({
			type :"post",
			dataType : "json" ,
			url : "/global/0nI0lMy6jAzAFRVe0DqLOw/trade/changeAlarm.do?idx="+idx+"&alarm="+alarm+"&kind=w",
			success:function(data){
				if(data.result == "suc"){
					location.reload();
				}
