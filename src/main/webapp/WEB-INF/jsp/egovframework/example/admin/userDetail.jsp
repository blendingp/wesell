							...
							<div class="card-body">
								<form name="updateForm" id="updateForm" method="post">
									...
									<div style="margin-bottom: 10px;">
										<button type="button"
											onclick="location.href='/wesell/admin/trade/orderList.do?searchSelect=idx&search=${info.idx}'"
											class="btn btn-primary">주문내역</button>
										<button type="button"
											onclick="location.href='/wesell/admin/trade/tradeList.do?searchSelect=idx&search=${info.idx}'"
											class="btn btn-primary">거래내역</button>
										<button type="button"
											onclick="location.href='/wesell/admin/trade/liqList.do?searchSelect=idx&search=${info.idx}'"
											class="btn btn-primary">청산내역</button>
										<c:if test="${adminLevel eq 1}">
											<button type="button"
												onclick="location.href='/wesell/admin/trade/positionList.do?search=${info.name}'"
												class="btn btn-primary">포지션내역</button>
										</c:if>
										<c:if test="${project.coinDeposit eq true}">
											<button type="button"
												onclick="location.href='/wesell/admin/transactions.do?pageIndex=1&searchSelect=idx&search=${info.idx}&label=%2B'"
												class="btn btn-primary">입금내역</button>
											<button type="button"
												onclick="location.href='/wesell/admin/transactions.do?pageIndex=1&searchSelect=idx&search=${info.idx}&label=-'"
												class="btn btn-primary">출금내역</button>
										</c:if>
										<c:if test="${project.krwDeposit eq true}">
											<button type="button"
												onclick="location.href='/wesell/admin/trade/kWithdrawalList.do?pageIndex=1&kind=d&stat=0<c:if test="${info.istest eq 1}">&test=test</c:if>&searchSelect=idx&search=${info.idx}'"
												class="btn btn-primary">입금내역(한화)</button>
											<button type="button"
												onclick="location.href='/wesell/admin/trade/kWithdrawalList.do?pageIndex=1&kind=w&stat=0<c:if test="${info.istest eq 1}">&test=test</c:if>&searchSelect=idx&search=${info.idx}'"
												class="btn btn-primary">출금내역(한화)</button>
										</c:if>
										<button type="button"
											onclick="location.href='/wesell/admin/contact/contactList.do?searchSelect=m.phone&search=${info.phone}'"
												...
												<div class="form-group">
													...
													<div class="form-group input-group">
														...
														<c:if test="${info.fkey ne null}">
															<span class="input-group-btn">
																<button type="button" onclick="location.href='/wesell/admin/user/kycInfo.do?idx=${info.idx}'" class="btn btn-info">확인</button>
															</span>
														</c:if>
														<span class="input-group-btn">
										...
										<div class="col-lg-4">
											<div class="form-group">
												<label>이메일 <c:if test="${info.emailconfirm eq 1}">(인증완료)</c:if>
													...
													href="/wesell/admin/trade/emailList.do?idx=${info.idx }">변경기록</a>
												<c:if
													test="${project.subAdminPower eq false and adminLevel ne 1}">
													<div class="form-group">
									...
									<div class="row">
										...
										<div class="col-lg-3">
											<div class="form-group">
												<label> 
<%-- 													<a href="/wesell/admin/account/realBalanceLog.do?coinname=XRP&kind=1&useridx=${info.idx}" target="_blank"></a> --%>
												...
												<div class="col-lg-2">
													<div class="form-group">
														<label> ${fn:toUpperCase(item.key)} Balance <c:if
																...
																<a
																	href="/wesell/admin/account/totalLog.do?userIdx=${info.idx}"
										...
										<div class="col-lg-12">
											<div class="row">
												<div class="col-lg-6">
													<div class="form-group">
														<label>상위</label>
														<pre><c:forEach var="item" items="${parents}" varStatus="i"><a href="/wesell/admin/user/userDetail.do?idx=${item.userIdx}">${item.name}(${item.level})</a><- </c:forEach>${info.name}(${info.level })</pre>
													</div>
												</div>
												<div class="col-lg-2">
													<div class="form-group">
	...
	총1 = 본인 60% - 총2 40% = 20%
				...
				<button type="button" onclick="location.href='/wesell/admin/user/userList.do'" class="btn btn btn-secondary">
					목록
				</button>
				<br /> <br />
			</div>
	...
	function userDanger(idx , danger){
		...
		if(confirm(txt)){
			...
			$.ajax({
				type :'post',
				url : '/wesell/admin/user/userDanger.do',
				data : data ,
	...
	function change(type , kind ,self){
		$.ajax({
			...
			url:'/wesell/admin/user/changeLevelOption.do',
			success:function(data){
	...
	function updateLevel(idx, level){
			$.ajax({
				...
				url:'/wesell/admin/user/updateLevel.do',
				dataType:'json',
				success:function(data){
					alert(data.msg);
					if(data.result == 'success'){
	...
	function updatePoint(idx , kind){
		...
		$.ajax({
			...
			url:'/wesell/admin/user/updatePoint.do',
			dataType:'json',
			success:function(data){
				alert(data.msg);
				if(data.result == 'success'){
	...
	function depositUSDT(idx){
		...
		$.ajax({
			...
			url:'/wesell/admin/user/depositUSDT.do',
			dataType:'json',
			success:function(data){
				alert(data.msg);
				if(data.result == 'success'){
	...
	function updateCommissionRate(idx){
		...
		$.ajax({
			...
			url:'/wesell/admin/user/updateCommissionRate.do',
			dataType:'json',
			success:function(data){
				alert(data.msg);
				if(data.result == 'success'){
	...
	function ipBan(idx){
		...
		$.ajax({
			type:'post',
			data:{"idx" : idx , "ip" : ip},
			url:'/wesell/admin/user/ipBan.do',
			dataType:'json',
			success:function(data){
				alert(data.msg);
				if(data.result == 'success'){
	...
	function userBan(idx){
		$.ajax({
			type:'post',
			data:{"idx" : idx},
			url:'/wesell/admin/user/userBan.do',
			dataType:'json',
			success:function(data){
				alert(data.msg);
				if(data.result == 'success'){
	...
	function releaseUserBan(idx){
		$.ajax({
			type:'post',
			data:{"idx" : idx},
			url:'/wesell/admin/user/releaseUserBan.do',
			dataType:'json',
			success:function(data){
				alert(data.msg);
				if(data.result == 'success'){
...
// 	}
	...
	function listChange(node,kind){
		...
		if(kind == "deposit"){
			...
			$("#listlink").attr("href","/wesell/admin/transactions.do?searchIdx="+"${info.idx}"+"&label=%2B"+"&username="+"${info.name}");
			$("#listlink").html("입금내역 전체보기");
		}
		else if(kind == "withdraw"){
			$("#wlist").css("display","flex");
			$("#listlink").attr("href","/wesell/admin/transactions.do?searchIdx="+"${info.idx}"+"&label=-"+"&username="+"${info.name}");
			$("#listlink").html("출금내역 전체보기");
		}
		else if(kind == "trade"){
			$("#tlist").css("display","flex");
			$("#listlink").attr("href","/wesell/admin/trade/tradeList.do?searchIdx="+"${info.idx}"+"&username="+"${info.name}");
	...
	function updateInfo(kind){ // all - 전체변경, 혹은 하나만 변경
		...
		$.ajax({
			type :'post',
			data : data,
			url : '/wesell/admin/user/updateInfo.do?kind='+$("#kind").val(),
	...
	function memberDelete(idx){ // all - 전체변경, 혹은 하나만 변경
		...
		$.ajax({
			type:'post',
			data:{"idx" : idx},
			url:'/wesell/admin/user/deleteUser.do',
			dataType:'json',
			success:function(data){
				alert(data.msg);
				if(data.result == 'success'){
	...
	function parentChange(parentIdx){
		...
		$.ajax({
			...
			url:'/wesell/admin/user/parentChange.do',
			dataType:'json',
			success:function(data){
				alert(data.msg);
				if(data.result == 'suc')
	...
	function changeJstat(idx , kind){
		if(confirm("변경하시겠습니까?")){
			$.ajax({
				type : 'get',
				url : '/wesell/admin/user/changeJstat.do?idx='+idx+'&jstat='+kind,
				success:function(data){
					alert(data.msg);
					location.reload();
				}
	...
	function setKrCode(idx){
		if(confirm("변경하시겠습니까?")){
			$.ajax({
				type : 'get',
				url : '/wesell/admin/user/setKrCode.do?idx='+idx,
				success:function(data){
					alert(data.msg);
					location.reload();
				}
	...
	function vConfirm(idx,vcf){
		if(confirm("변경하시겠습니까?")){
			$.ajax({
				type : 'get',
				url : '/wesell/admin/user/setVConfirm.do?idx='+idx+"&vConfirm="+vcf,
				success:function(data){
					alert(data.msg);
					location.reload();
				}
	...
	function kycConfirm(idx , cf){
		...
		$.ajax({
			type : 'get',
			url : '/wesell/admin/user/kycConfirm.do?idx='+idx+'&confirm='+cf+"&text="+text,
			success:function(data){
				alert(data.msg);
				location.reload();
			}
