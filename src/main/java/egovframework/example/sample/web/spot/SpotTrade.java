...
public class SpotTrade {
	...
	public SpotTrade(Member mem, String _symbol, String _orderType, String _position, double _buyQuantity){
		userIdx = 0; 
		leverage = 1; // 배당
		try{
	...
	public SpotTrade(String _orderNum,int _userIdx, String _symbol, String _orderType, String _position, double _buyQuantity, int _leverage, String _marginType){
		try{
			userIdx = _userIdx;
			orderNum = _orderNum;
			leverage = _leverage;
	...
	public SpotTrade(Member mem, String _symbol, String _orderType, String _position, double _buyQuantity, int _leverage, String _marginType){
		try{
			userIdx = mem.userIdx;
			orderNum = mem.getOrderNum(); //주문 고유 번호
			leverage = _leverage;
	...
	public void insertTradeLog(double fee, double entryPrice){ // tradelog DB insert
    	try {
    		Log.print("SPOT: call insertTradeLog", 5, "call");
    		...
    		QueryWait.pushQuery("insertSpotTradeLog",userIdx, in, QueryType.INSERT);
		} catch (Exception e) {
			Log.print("SPOT: insertTradeLog err! "+e, 0, "err");
		}
    }
